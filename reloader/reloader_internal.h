...
#include "feature_engine/reloader/reloader_parser.h"

namespace feature_engine {
namespace reloader {

using commonlib::RedisClusterClient;
using commonlib::StringUtils;
...
class SetDB : public DBInterface<KeyType, int> {
 public:
  SetDB() {
    _db = std::make_shared<std::unordered_set<KeyType>>();
  }

  std::shared_ptr<std::unordered_set<KeyType>> GetDB() {
    return _db;
  }

 private:
  std::shared_ptr<std::unordered_set<KeyType>> _db;
};
...
class VectorReloader {
 public:
  VectorReloader(ReloaderMeta& reloader_meta)
    ...
    _common_db = std::make_shared<VectorDB<KeyType, std::string>>();
  }
...
template<typename KeyType, typename ParserType>
class SimpleSetReloader {
 public:
  SimpleSetReloader(ReloaderMeta& reloader_meta)
      : file_name_(reloader_meta.file_name) {
    _common_db = std::make_shared<SetDB<KeyType>>();
  }
  typedef std::unordered_set<KeyType> DBType;
  std::shared_ptr<DBType> GetDB() { return _common_db->GetDB(); }
  ...
  int Load() {
    ...
    while (std::getline(ifs, line)) {
      if (line.empty()) { continue; }
      if (parser.Parse(line, key) != PARSER_OK) {
        continue;
      }
      db->insert((key));
    }
 ...
 private:
  std::shared_ptr<SetDB<KeyType>> _common_db;
  std::string file_name_;
};

template<typename KeyType, typename ParserType>
class SimpleVectorReloader {
 public:
  SimpleVectorReloader(ReloaderMeta& reloader_meta)
      : file_name_(reloader_meta.file_name) {
    _common_db = std::make_shared<VectorDB<KeyType, std::string>>();
  }
  typedef std::vector<KeyType> DBType;
 ...
 private:
  std::shared_ptr<VectorDB<KeyType, std::string>> _common_db;
  std::string file_name_;
};
...
class VectorRedisReloader {
 public:
  ...
  VectorRedisReloader(ReloaderMeta& reloader_meta)
    ...
    redis_client_ = std::make_unique<RedisClusterClient>(
 ...
 private:
  ...
  std::unique_ptr<RedisClusterClient> redis_client_;
...
template<typename KeyType, typename ValueType, typename ParserType>
class MapRedisReloader {
 public:
  typedef MapDB<KeyType, ValueType> MapType;
  typedef std::unordered_map<KeyType, ValueType> DBType;
  MapRedisReloader(ReloaderMeta& reloader_meta)
      : kv_sep_(reloader_meta.key_seprator),
        val_sep_(reloader_meta.value_seprator),
        redis_key_(reloader_meta.redis_key) {
    _common_db = std::make_shared<MapType>();
